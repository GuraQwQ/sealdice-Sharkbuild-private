...
// CallOnGroupUpload calls OnGroupUpload through wrapper indirection.
func (i *ExtInfo) CallOnGroupUpload(d *Dice, ctx *MsgContext, msg *Message, file *GroupUploadFile) {
	ext := i.GetRealExt()
	if ext == nil || ext.OnGroupUpload == nil {
		return
	}
	ext.callWithJsCheck(d, func() {
...
func (group *GroupInfo) SyncExtensionsOnMessage(d *Dice) {
	...
	for _, ext := range d.ExtList {
		...
		if ext != nil && (ext.AutoActive || (ext.DefaultSetting != nil && ext.DefaultSetting.AutoActive)) {
			group.extActivateInternal(ext, ActivateReasonFirstMessage)
		} else {
			group.AddToInactivated(ext.Name)
		}
