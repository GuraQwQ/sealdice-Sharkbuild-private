...
// OnGroupUpload handles OneBot 11 group_upload notices.
func (s *IMSession) OnGroupUpload(ctx *MsgContext, msg *Message, file *GroupUploadFile) {
	...
	groupInfo, ok := s.ServiceAtNew.Load(msg.GroupID)
	if !ok {
		s.Parent.Logger.Debugf("group_upload: group %s not found, skip extension hooks", msg.GroupID)
		return
	}
	ctx.Group = groupInfo
	if ctx.Player == nil {
	...
	for _, ext := range groupInfo.GetActivatedExtList(ctx.Dice) {
		ext.CallOnGroupUpload(ctx.Dice, ctx, msg, file)
	}
}
