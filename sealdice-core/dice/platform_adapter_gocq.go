...
func firstGocqValue(root gjson.Result, paths ...string) gjson.Result {
...
func firstGocqObject(root gjson.Result, paths ...string) gjson.Result {
	for _, path := range paths {
		value := root.Get(path)
...
func (pa *PlatformAdapterGocq) tryHandleGroupUpload(message string, ep *EndPointInfo, session *IMSession, log *zap.SugaredLogger) bool {
	if !gjson.Valid(message) {
		return false
	}
	...
	isGroupUpload := (root.Get("post_type").String() == "notice" && root.Get("notice_type").String() == "group_upload") ||
		root.Get("detail_type").String() == "notice.group_upload" ||
		(root.Get("type").String() == "notice" && root.Get("detail_type").String() == "group_upload")
	if !isGroupUpload {
		return false
	}

	groupIDRaw := firstGocqValue(root, "group_id", "data.group_id", "detail.group_id")
	userIDRaw := firstGocqValue(root, "user_id", "data.user_id", "detail.user_id")
	fileObj := firstGocqObject(root, "file", "data.file", "detail.file")
	...
	if fileName == "" {
		fileName = fileObj.Get("file_name").String()
	}
	log.Infof("group_upload event: group_id=%s user_id=%s file=%s", groupID, userID, fileName)
...
func (pa *PlatformAdapterGocq) Serve() int {
	...
	socket.OnTextMessage = func(message string, socket gowebsocket.Socket) {
		...
		if pa.tryHandleGroupUpload(message, ep, session, log) {
			return
		}
